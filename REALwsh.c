...
// Function to find a background job by its job ID
struct Job* findBackgroundJobByJobID(int job_id) {
    for (int i = 0; i < background_jobs_size; i++) {
        if (background_jobs[i]->job_id == job_id) {
...
void sigint_handler(int signum) {
    ...
    // Typically, you would send the signal to the foreground process group
    // printf("BEFORE KILLING: %d\n", CURRENT_JOB);

    if(CURRENT_JOB_ID != 0) {
        kill(-CURRENT_JOB_ID, SIGINT);
    }
}
...
void sigtstp_handler(int signum) {
    ...
    if(CURRENT_JOB_ID != 0) {
        kill(-CURRENT_JOB_ID, SIGTSTP);
        addToBackgroundJobs(CURRENT_JOB);  
    } 
}
...
int main(int argc, char *argv[]) {
    ...
    SHELL_PID = getpgid(0);
    // printf("PGID OF SHELL: %d\n", SHELL_PID);

        FILE *input_file = stdin; // Default to reading from stdin

    // Check if two arguments are provided; if so, open the file for reading
    if (argc == 2) {
    ...
    while (1) {
        ...
        ssize_t read = getline(&input_line, &line_size, input_file);
        if (read == -1) {
            ...
            if (strcmp(job.processes[0].command, "exit") == 0) {
                ...
                if(background_jobs_size > 0 ) {
                    ...
                    if (job.processes[0].args[1] != NULL) {
                        ...
                        temp = largest_job_id;
                        // printf("largest id value: %d\n", temp);
                    }
                    // printf("job ID background: %d\n", background_jobs[temp-1]->job_id);
                    // printf("PGID IN BACK: %d\n", background_jobs[temp-1]->pgid);

                    // tcsetpgrp(STDIN_FILENO, background_jobs[temp-1]->pgid);
                    // Resume the job by sending it the SIGCONT signal
                    kill(-background_jobs[temp-1]->pgid, SIGCONT);
                    int status;
                    waitpid(background_jobs[temp-1]->pgid, &status, WUNTRACED);
                ...
                if (pid < 0) {
                    ...
                    // Parent process
                    job.pgid = pid;
                    // printf("PGID WHILE RUNNIGN: %d\n", job.pgid);
                    // printf("job ID WHILE RUNNIGN: %d\n", job.job_id);
                    if (!job.background) {
                        //printf("PGID OF PARENT: %d\n", getpgid(0));
                        CURRENT_JOB_ID = pid;
                        CURRENT_JOB = &job;
